...
public class JobsServerPacket {
    ...
    public static void writeS2CJobPacket(JobsManager jobsManager, ServerPlayerEntity serverPlayerEntity) {
        ServerPlayNetworking.send(serverPlayerEntity, new JobPacket(new JobPacket.JobRecord(jobsManager.getPlayerJobs().values().stream().toList()), jobsManager.getEmployedJobsList(), jobsManager.getEmployedJobTime()));
    }
